...
const WeatherChart = ({
    ...
    try {
      ...
      const filteredData = date
        ? result.list.filter((item) => {
            const itemDate = new Date(item.dt_txt.split(" ")[0]).setHours(
              0,
              0,
              0,
              0
      ...
      const uniqueDatesSet = new Set(result.list.map((curelem) => {
        const dateObject = new Date(curelem.dt_txt);
  ...
  useEffect(() => {
    if (!isOnline) {
      ...
      setError(
        "An error occurred while fetching weather data. Try to enter City name correctly"
      );
    }
        ...
        {isLoading || isTyping ? (
          ...
          <>
            {!error && data.length !== 0 ? (
              ...
              <div className="error">
                {isOnline && error && place !== "" ? error : handleApiError()}
              </div>
            )}
            <SelectDate
              data={data}
